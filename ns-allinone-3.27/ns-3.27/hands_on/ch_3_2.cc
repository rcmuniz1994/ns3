...
for (int i=1;i<=3;i++)
{
    echoClient.SetAttribute ("PacketSize", UintegerValue (512*i));

    ApplicationContainer clientApps = echoClient.Install (wifiStaNodes.Get (nWifi -i));

    clientApps.Start (Seconds (5.0*i));
...
AnimationInterface anim ("anim_ch_3.xml");
...
anim.SetConstantPosition(wifiStaNodes.Get(4), 6.0, 7.0);
anim.SetConstantPosition(wifiStaNodes.Get(3), 7.0, 8.0);
anim.SetConstantPosition(wifiStaNodes.Get(2), 8.0, 9.0);
